<x-header-page data-href="{{ route('admin.paginas.agenda.comemoracoes.index') }}" placeholder="Pesquisar eventos..." title="Adicionar Evento">
...
<x-slot:form action="{{ route('admin.paginas.agenda.comemoracoes.post') }}" method="post" style="{{ $errors->any() || request('id') ? 'display: block; transform: translateY(-100%);' : 'display: none; transform: translateY(0%);' }}" enctype="multipart/form-data" autocomplete="off" novalidate>

	@csrf

	{{-- <input type="hidden" name="categoria" value="agenda"> --}}
	{{-- <input type="hidden" name="tipo" value="A"> --}}
	<input type="hidden" name="tipo" value="comemoracao">
	...
	@include('admin.paginas.agenda.comemoracoes.includes.scripts')
